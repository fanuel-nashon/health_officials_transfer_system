<x-app-layout>
    <x-slot name="header">District Health Officer Dashboard</x-slot>

    <div class="mt-2 space-y-6">
        <div class="bg-blue-50 border border-blue-200 rounded-xl px-5 py-4">
            <p class="text-sm text-blue-700">
                District: <strong>{{ $district }}</strong> &mdash;
                <strong>{{ $pendingCount }}</strong> transfer(s) awaiting district-level review.
            </p>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 class="font-semibold text-gray-700">Pending District Reviews</h2>
                <a href="{{ route('transfers.index') }}" class="text-sm text-green-600 hover:text-green-800">All Transfers</a>
            </div>
            @if($pending->isEmpty())
                <p class="px-6 py-10 text-center text-sm text-gray-400">No transfers awaiting district-level review.</p>
            @else
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50 text-xs text-gray-500 uppercase tracking-wider">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left">Facility Comment</th>
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            @foreach($pending as $t)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-3 text-gray-500 text-xs italic max-w-xs truncate">{{ $t->facility_comment ?? '—' }}</td>
