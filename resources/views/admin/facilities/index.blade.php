    ...
    <div class="mt-2">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="font-semibold text-gray-700">Health Facilities</h2>
            </div>
            @if($facilities->isEmpty())
                <p class="px-6 py-10 text-center text-sm text-gray-400">No facilities registered.</p>
            @else
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            @foreach($facilities as $facility)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-3 font-medium text-gray-800">{{ $facility->name }}</td>
                                    <td class="px-6 py-3 text-gray-600">{{ optional($facility->location)->name ?? '—' }}</td>
                                    <td class="px-6 py-3 text-gray-600">{{ optional($facility->location)->district ?? '—' }}</td>
                                    <td class="px-6 py-3 text-gray-600">{{ optional($facility->location)->region ?? '—' }}</td>
